...
## 2026-09-02 - [Interactive Effect Screen-Reader Parity]
**Learning:** In Reigns-style choice interfaces, screen-reader users miss out on visual hover feedback (like pinging dots on resource bars) that hint at the consequences of a choice before it's made.
**Action:** Inject visually hidden (`sr-only`) summaries of interactive effects directly into the choice buttons so screen readers announce the expected consequences on focus, mirroring the visual hover state.
