...
## 2025-03-02 - Screen Reader Parity for Hover-Driven Game Insights
**Learning:** In Reigns-style choice games, displaying stat changes strictly via visual hints (like pulsing dots on resource bars during button hover) excludes screen-reader users, depriving them of vital decision-making info.
**Action:** Always inject visually hidden (`sr-only`) summaries of anticipated consequences directly into the interactive choice buttons. This ensures screen readers announce the exact same cause-and-effect insight on focus that sighted users get on hover.
