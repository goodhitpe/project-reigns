...
## 2026-09-04 - Screen-reader parity for Reigns-style visual consequence indicators
**Learning:** In Reigns-style choice interfaces, visual hover indicators (e.g. pinging dots on resources) must have screen-reader parity. Otherwise, visually impaired users cannot anticipate the consequences of their choices.
**Action:** Inject visually hidden (`sr-only`) summaries of interactive effects so screen readers can announce expected consequences on focus, mirroring visual hover feedback.
