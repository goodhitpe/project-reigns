...
## 2025-03-02 - Screen-reader Parity in Choice Interfaces
**Learning:** In Reigns-style choice interfaces, visualizing which stats will change when hovering over a choice is a key game mechanic. However, screen reader users miss this critical information if it's only shown visually or via ping animations on the resource bars.
**Action:** Ensure screen-reader parity by injecting visually hidden (`sr-only`) summaries of interactive effects (e.g. "예상 변동: 예산, 일정") into the choice buttons themselves, so they are announced along with the button's action.
