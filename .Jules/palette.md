...
## 2025-03-05 - Screen-Reader Parity for Hover Previews
**Learning:** In Reigns-style choice interfaces, hover states often provide visual clues about the consequences of a decision (e.g., which resource gauges will fluctuate). Without screen reader parity, visually impaired users are deprived of this crucial predictive gameplay information.
**Action:** When visual indicators hint at consequences on hover/focus, inject visually hidden (`sr-only`) summaries inside the interactive choice elements so screen readers narrate the expected outcomes alongside the choice text.
